...
pub fn main() !void {
    ...
    // for (0..8) |lines| {
    //     for (0..128) |cols| {
    //         try ssd1306.wt.writeAll(&[_]u8{ 0x40, if (lines == 0 or cols <= 7) 0xFF else 0x00 }); // or lines == 7  or cols >= 120
    //     }
    // }
    ...
    try ssd1306.setContrast(1);
    busyloop(2_000_000);

    try ssd1306.setContrast(255);
    busyloop(2_000_000);

    try ssd1306.setCOMOuputScanDirection(false); // Flip up/down
    busyloop(2_000_000);

    try ssd1306.setCOMOuputScanDirection(true); // Flip up/down
    busyloop(2_000_000);

    try uart.writer().writeAll("Setup scrolling\r\n");
    try ssd1306.deactivateScroll();
    try ssd1306.setVerticalScrollArea(8, 48);
    try ssd1306.continuousHorizontalScrollSetup(.left, 1, 6, 0b111);
